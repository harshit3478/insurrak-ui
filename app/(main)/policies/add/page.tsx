"use client";

import { useActionState } from "react";
import { useRouter } from "next/navigation";
import { usePolicies } from "@/context-provider/PolicyProvider";
import { PolicyForm } from "@/components/Policies/PolicyForm";
import { FormHeader, SuccessHeader } from "@/components/ui/FormCommon";
import { FileText, ArrowRight, List } from "lucide-react";
import { initialState } from "@/types";

export default function AddPolicyPage() {
  const { createPolicyAction } = usePolicies();
  const [createState, createPolicy, isCreating] = useActionState(createPolicyAction, initialState);
  const router = useRouter();

  if (createState.success) {
    const data = createState.data || {};
    return (
      <div className="p-8 bg-[#F4F7FE] dark:bg-gray-dark min-h-screen font-sans">
        <SuccessHeader
          title="Policy Request Created Successfully!"
          subtitle="The request has been submitted and is now in the review workflow."
        />

        <div className="bg-white dark:bg-gray-dark rounded-2xl border border-gray-200 dark:border-dark-3 shadow-sm overflow-hidden mb-8">
          <div className="p-6 border-b border-gray-100 dark:border-dark-3 bg-gray-50/50 dark:bg-dark-2 flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-[#0B1727] flex items-center justify-center text-white">
              <FileText className="w-5 h-5" />
            </div>
            <h2 className="text-lg font-bold text-gray-900 dark:text-white">Registration Summary</h2>
          </div>

          <div className="p-8 grid grid-cols-1 md:grid-cols-3 gap-12">
            {/* Column 1: Core Info */}
            <div>
              <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-4">Policy Info</h3>
              <div className="space-y-4">
                <div>
                  <div className="text-xs text-gray-400 mb-0.5">Policy Number</div>
                  <div className="text-sm font-medium text-gray-900 dark:text-white">{data.policy_number || "TBD"}</div>
                </div>
                <div>
                  <div className="text-xs text-gray-400 mb-0.5">Line of Business</div>
                  <div className="text-sm font-medium text-gray-900 dark:text-white">{data.line_of_business}</div>
                </div>
                <div>
                  <div className="text-xs text-gray-400 mb-0.5">Request ID</div>
                  <div className="text-sm font-medium text-blue-600 dark:text-blue-400">PR-{data.id}</div>
                </div>
              </div>
            </div>

            {/* Column 2: Financials */}
            <div>
              <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-4">Financial Details</h3>
              <div className="space-y-4">
                <div>
                  <div className="text-xs text-gray-400 mb-0.5">Sum Insured</div>
                  <div className="text-sm font-medium text-gray-900 dark:text-white">
                    {data.sum_insured ? `₹${Number(data.sum_insured).toLocaleString("en-IN")}` : "0"}
                  </div>
                </div>
                <div>
                  <div className="text-xs text-gray-400 mb-0.5">Estimated Premium</div>
                  <div className="text-sm font-medium text-gray-900 dark:text-white">
                    {data.premium ? `₹${Number(data.premium).toLocaleString("en-IN")}` : "0"}
                  </div>
                </div>
              </div>
            </div>

            {/* Column 3: Coverage */}
            <div>
              <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-4">Coverage Period</h3>
              <div className="space-y-4">
                <div>
                  <div className="text-xs text-gray-400 mb-0.5">Start Date</div>
                  <div className="text-sm font-medium text-gray-900 dark:text-white">{data.policy_start_date || "-"}</div>
                </div>
                <div>
                  <div className="text-xs text-gray-400 mb-0.5">End Date</div>
                  <div className="text-sm font-medium text-gray-900 dark:text-white">{data.policy_end_date || "-"}</div>
                </div>
                <div>
                  <div className="text-xs text-gray-400 mb-0.5">Workflow Status</div>
                  <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium bg-blue-50 text-blue-700">
                    {data.status}
                  </div>
                </div>
              </div>
            </div>
          </div>

          <div className="p-6 bg-gray-50/50 dark:bg-dark-2 border-t border-gray-100 dark:border-dark-3 flex justify-between items-center">
            <button
              onClick={() => router.push("/policies")}
              className="flex items-center gap-2 px-6 py-2.5 bg-[#0B1727] text-white rounded-lg font-medium hover:bg-[#1a2639] transition-colors shadow-sm"
            >
              <List className="w-4 h-4" /> Go to Policy List
            </button>
            <button
              onClick={() => router.push(`/policies/edit/${data.id}`)}
              className="flex items-center gap-2 text-sm font-medium text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white transition-colors"
            >
              View Full Details <ArrowRight className="w-4 h-4" />
            </button>
          </div>
        </div>
      </div>
    );
  }

  return (
    <div className="p-8 bg-[#F4F7FE] dark:bg-gray-dark min-h-screen font-sans">
      <FormHeader
        title="Add New Policy"
        subtitle="Submit a new policy request to initiate the insurance procurement workflow."
      />
      {createState.error && (
        <div className="mb-6 p-4 bg-red-50 text-red-600 rounded-lg text-sm border border-red-100">
          {createState.error}
        </div>
      )}
      <PolicyForm action={createPolicy} pending={isCreating} />
    </div>
  );
}
